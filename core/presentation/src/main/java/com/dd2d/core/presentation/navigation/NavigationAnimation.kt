...
fun defaultFadeIn(): EnterTransition =
  fadeIn(animationSpec = tween(DEFAULT_FADE_ANIMATION_DURATION))

fun defaultFadeOut(): ExitTransition =
  fadeOut(animationSpec = tween(DEFAULT_FADE_ANIMATION_DURATION))

fun defaultScaleIn(): EnterTransition = scaleIn(
  animationSpec = tween(DEFAULT_SLIDE_ANIMATION_DURATION),
  initialScale = 0.95F
)//, transformOrigin = DEFAULT_TRANSFORM_ORIGIN)

fun defaultScaleOut(): ExitTransition = scaleOut(
  animationSpec = tween(DEFAULT_SLIDE_ANIMATION_DURATION),
  targetScale = 0.95F
)//, transformOrigin = DEFAULT_TRANSFORM_ORIGIN)

fun defaultSlideInHorizontal(): EnterTransition =
  slideInHorizontally(animationSpec = tween(DEFAULT_SLIDE_ANIMATION_DURATION)) { it }

fun defaultSlideOutHorizontal(): ExitTransition =
  slideOutHorizontally(animationSpec = tween(DEFAULT_SLIDE_ANIMATION_DURATION)) { it }

fun defaultSlideInVertical(): EnterTransition =
  slideInVertically(animationSpec = tween(DEFAULT_SLIDE_ANIMATION_DURATION)) { it }

fun defaultSlideOutVertical(): ExitTransition =
  slideOutVertically(animationSpec = tween(DEFAULT_SLIDE_ANIMATION_DURATION)) { it }
