...
class CommonActionResultBus: ActionResultBus {
    ...
    suspend fun emitSuccess() { _actionResult.emit(CommonActionResult.ActionSuccess) }
    suspend fun emitFailure(exception: Throwable) { _actionResult.emit(CommonActionResult.ActionFailure(exception)) }
}
