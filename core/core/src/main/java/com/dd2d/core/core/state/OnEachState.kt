...
inline fun <reified T> Flow<DataState<T>>.onStateSuccess(crossinline block: suspend T.() -> Unit): Flow<DataState<T>> =
  this
    .onEach { state ->
...
inline fun <reified T> Flow<DataState<T>>.onStateLoading(crossinline block: suspend () -> Unit): Flow<DataState<T>> =
  this
    .onEach { state ->
...
inline fun <reified T> Flow<DataState<T>>.onStateError(crossinline block: suspend (exception: ManagedException) -> Unit): Flow<DataState<T>> =
  this
    .onEach { state ->
