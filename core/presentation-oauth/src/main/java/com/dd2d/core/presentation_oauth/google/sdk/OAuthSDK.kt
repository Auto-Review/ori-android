...
abstract class OAuthSDK {
    ...
    protected fun stateToError(exception: ManagedException) = _state.update { OAuthState.Error(exception = exception) }
    protected fun stateToSuccess(result: OAuthResult) = _state.update { OAuthState.Success(result = result) }
